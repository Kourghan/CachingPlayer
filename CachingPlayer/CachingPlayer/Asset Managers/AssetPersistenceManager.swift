...
class AssetPersistenceManager: NSObject {
    ...
    fileprivate var activeDownloadsMap = [AVAssetDownloadTask: AVURLAsset]()
    ...
    func restorePersistenceManager() {
        ...
        assetDownloadURLSession.getAllTasks { tasksArray in
            ...
            for task in tasksArray {
                guard let assetDownloadTask = task as? AVAssetDownloadTask else { break }

                let urlAsset = assetDownloadTask.urlAsset
                
                self.activeDownloadsMap[assetDownloadTask] = urlAsset
            }
    ...
    func downloadStream(for asset: AVURLAsset) {
        guard let task = assetDownloadURLSession.makeAssetDownloadTask(asset: asset,
        ...
        task.taskDescription = asset.description
...
//        NotificationCenter.default.post(name: .AssetDownloadStateChanged, object: nil, userInfo: userInfo)
    ...
    func assetForStream(withName name: String) -> AVURLAsset? {
        var asset: AVURLAsset?

        for (_, assetValue) in activeDownloadsMap where name == assetValue.description {
            asset = assetValue
            break
        }
...
//        userInfo[Asset.Keys.name] = asset.stream.name
        ...
        if let error = error as NSError? {
            ...
            case (NSURLErrorDomain, NSURLErrorCancelled):
                 ...
                 */
                guard let localFileLocation = localAssetForStream(withName: asset.description)?.url else { return }
...
//                    userDefaults.removeObject(forKey: asset.stream.name)
                    ...
                    print("An error occured trying to delete the contents on disk for \(asset.description): \(error)")
                }
